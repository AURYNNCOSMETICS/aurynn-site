export default function App() {
  ...
  return (
    <main className="min-h-screen w-full bg-[var(--base)] text-[var(--ink)]" style={{
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-[var(--base)]/80 border-b border-[var(--sand)]/60">
        <div className="mx-auto max-w-6xl px-4 py-4 flex items-center justify-between">
          <a href="#home" className="tracking-[0.25em] text-xl md:text-2xl font-serif" aria-label="Aurynn home">
            {brand.name}
          </a>
          <nav className="hidden md:flex gap-8 text-sm">
            <a href="#collectie" className="hover:text-[var(--gold)] transition">Collectie</a>
            <a href="#verhaal" className="hover:text-[var(--gold)] transition">Ons verhaal</a>
          ...
          <a href="#shop" className="md:inline-flex hidden rounded-full border px-4 py-2 text-sm border-[var(--gold)] text-[var(--ink)] hover:bg-[var(--gold)] hover:text-white transition" aria-label="Shop nu">Shop nu</a>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-6xl px-4 py-24 md:py-28 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-8 flex gap-4">
              <a href="#collectie" className="rounded-full bg-[var(--ink)] text-white px-6 py-3 text-sm tracking-wide hover:bg-[var(--gold)] transition">
                Ontdek collectie
              </a>
              <a href="#verhaal" className="rounded-full border border-[var(--sand)] px-6 py-3 text-sm tracking-wide hover:border-[var(--gold)] transition">
                Lees ons verhaal
              </a>
            </div>
          </div>
          <div className="relative aspect-[4/3] rounded-3xl border border-[var(--sand)] overflow-hidden shadow-sm">
            <img alt="Rustige lifestyle" className="h-full w-full object-cover" src="https://images.unsplash.com/photo-1600180758890-6b94519a8ba6?q=80&w=1600&auto=format&fit=crop" />
          </div>
        </div>
      </section>
      ...
      <section id="collectie" className="mx-auto max-w-6xl px-4 py-20">
        <div className="mb-10">
          ...
          <p className="mt-3 text-[var(--ink)]/80 max-w-prose">Start met twee zorgvuldig geselecteerde favorieten. Meer komt binnenkort.</p>
        </div>
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-6 flex items-center justify-between">
                  ...
                  <div className="flex gap-3">
                    <a href={p.url} className="rounded-full bg-[var(--gold)] text-white px-5 py-2 text-sm hover:brightness-110 transition" onClick={(e)=>{e.preventDefault(); alert('Koppelen aan Shopify-productURL zodra beschikbaar.');}}>Koop nu</a>
                    <button className="rounded-full border border-[var(--sand)] px-5 py-2 text-sm hover:border-[var(--gold)] transition" onClick={()=>alert('Toegevoegd aan (demo) winkelmand. Koppelen aan echte cart later.')}>Voeg toe</button>
                  </div>
                </div>
              </div>
            </article>
        ...
        <div className="mx-auto max-w-6xl px-4 py-20 grid md:grid-cols-2 gap-10 items-center">
          <div className="order-last md:order-first">
            <div className="relative aspect-[4/3] rounded-3xl border border-[var(--sand)] overflow-hidden">
              <img alt="Zachte materialen en kalme styling" className="h-full w-full object-cover" src="https://images.unsplash.com/photo-1493663284031-b7e3aefcae8e?q=80&w=1600&auto=format&fit=crop" />
            </div>
          </div>
          <div>
            ...
            <p className="mt-4 text-[var(--ink)]/80 leading-relaxed">
              Bij Aurynn geloven we dat kleine rituelen het verschil maken. Met een focus op rustige esthetiek,
              prettige materialen en zorgvuldige details brengen we producten die je helpen vertragen — elke dag opnieuw.
            </p>
            <div className="mt-6 flex gap-3">
              <a href="#collectie" className="rounded-full bg-[var(--ink)] text-white px-6 py-3 text-sm tracking-wide hover:bg-[var(--gold)] transition">Shop de collectie</a>
              <a href="#contact" className="rounded-full border border-[var(--sand)] px-6 py-3 text-sm tracking-wide hover:border-[var(--gold)] transition">Contact</a>
            </div>
          </div>
        </div>
      </section>
          ...
          {[{
            ...
            <details key={i} className="group py-4">
              <summary className="cursor-pointer list-none flex items-center justify-between text-lg font-medium">
                <span>{item.q}</span>
                <span className="ml-4 text-[var(--gold)] group-open:rotate-45 transition">＋</span>
              </summary>
      ...
      <footer id="contact" className="border-t border-[var(--sand)]/60">
        <div className="mx-auto max-w-6xl px-4 py-12 grid md:grid-cols-3 gap-8">
          <div>
            ...
            <p className="mt-3 text-sm text-[var(--ink)]/70 max-w-sm">Calm. Glow. Repeat. Wellness & beauty gifts for peaceful living.</p>
          </div>
          <form onSubmit={(e)=>{e.preventDefault(); alert('Nieuwsbrief ingediend – koppel later aan e-mail provider.');}} className="md:col-span-2">
            <label className="text-sm text-[var(--ink)]/70">Nieuwsbrief</label>
            <div className="mt-2 flex gap-3">
              <input required placeholder="E-mailadres" className="flex-1 rounded-full border border-[var(--sand)] bg-white/70 px-4 py-3 outline-none focus:border-[var(--gold)]" />
              <button className="rounded-full bg-[var(--gold)] text-white px-6 py-3 text-sm hover:brightness-110">Inschrijven</button>
            </div>
          </form>
        </div>
        <div className="text-center text-xs text-[var(--ink)]/60 py-6">© {new Date().getFullYear()} Aurynn. Alle rechten voorbehouden.</div>
      </footer>
    </main>
  );
}
